                ...
                client.on('guildMemberAdd', async member => {
                    ...
                    if(serverLock) {
                        try{
                            ...
                            .setColor(`${config.logging_config.serverlock_logs_color || config.main_config.colorhex}`)
                            .setTitle(`⚠️  Notification  ⚠️`)
                            .setAuthor(`${member.user.tag}`, `${member.user.displayAvatarURL({dynamic: true})}`, `${config["other_configuration"].serverinvite}`)
