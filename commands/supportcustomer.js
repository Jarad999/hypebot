module.exports = {
    ...
    aliases: ['csupport', 'customersupport', 'paidsupport', 'paidproductshelp', 'customerchans', 'customerchannels', 'cwrongchannels', 'customerhelp'],
    async execute(client, message, args, Hyperz, config, con){
        ...
        if(config["assistant_config"].enabled) {
            ...
            if(config["assistant_config"].useSupportCommands) {
                ...
                message.channel.send(config["assistant_config"].customerSupportMessage).then(msg => {msg.delete({ timeout: 40000 })}).catch(e => {if(config["main_config"].debugmode) return console.log(e);})
