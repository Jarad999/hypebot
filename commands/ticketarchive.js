...
module.exports = {
    ...
    async execute(client, message, args, Hyperz, config, con) {
        ...
        if (config["tickets_config"].enabled !== true) return message.channel.send("This module is disabled, please ask the owner to enable it!").then(msg => msg.delete({ timeout: 9000 })).catch(e => {if(config["main_config"].debugmode) return console.log(e);})
                ...
                if (react23847.emoji.name === '✅') {
                    ...
                    message.channel.messages.fetch({ limit: 100, before: message.id }).then(async(collected) => {
                        ...
                        var embed = new MessageEmbed()
                            ...
                            .setColor(`${config.logging_config.ticketarchive_logs_color || config.main_config.colorhex}`)
