...
module.exports = {
  ...
  const permissions = message.channel.permissionsFor(message.client.user);
    ...
    try {
      ...
      message.channel.send(error.message).catch(e => {if(config["main_config"].debugmode) return console.log(e);});
    }
    ...
    collector.on("collect", async (reaction, user) => {
      try {
        ...
        return message.channel.send(error.message).catch(e => {if(config["main_config"].debugmode) return console.log(e);});
      }
    });
  } else {
...
function generateQueueEmbed(message, queue) {
  ...
  for (let i = 0; i < queue.length; i += 10) {
    ...
    const embed = new MessageEmbed()
     .setAuthor("Server Songs Queue", "https://cdn.discordapp.com/attachments/743483303281295370/802321180287565854/Music.gif")
    ...
    .setFooter("Currently Server Volume is "+serverQueue.volume)
     if(serverQueue.songs.length === 1)embed.setDescription(`No songs to play next add songs by \`\`${config["main_config"].prefix}play <song_name>\`\``)
