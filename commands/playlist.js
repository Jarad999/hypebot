    ...
    async execute(client, message, args, Hyperz, config, con){
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			try {
				...
				if(config["main_config"].debugmode) return console.log(error);
				return sendError("Playlist not found :(",message.channel).catch(e => {if(config["main_config"].debugmode) return console.log(e);});
			}
		} else {
			try {
				...
				let thing = new MessageEmbed()
					.setAuthor("Playlist has been added to queue", "https://cdn.discordapp.com/attachments/743483303281295370/802321180287565854/Music.gif")
					.setThumbnail(songInfo.thumbnail)
					.setColor("GREEN")
					.setDescription(`✅  **|**  Playlist: **\`${songInfo.title}\`** has been added \`${songInfo.videoCount}\` video to the queue`)
				return message.channel.send(thing)
			} catch (error) {
				return sendError("An unexpected error has occurred",message.channel).catch(e => {if(config["main_config"].debugmode) return console.log(e);});
			}
		}
		...
		async function handleVideo(video, message, channel, playlist = false) {
			...
			if (!serverQueue) {
				...
				if (playlist) return;
				let thing = new MessageEmbed()
					.setAuthor("Song has been added to queue", "https://cdn.discordapp.com/attachments/743483303281295370/802321180287565854/Music.gif")
...
let thing = new MessageEmbed()
				.setAuthor("Started Playing Music!", "https://cdn.discordapp.com/attachments/743483303281295370/802321180287565854/Music.gif")
				.setThumbnail(song.img)
				.setColor(`${config.main_config.colorhex}`)
				.addField("Name", song.title, true)
				.addField("Duration", song.duration, true)
