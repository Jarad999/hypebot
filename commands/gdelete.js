...
module.exports = {
    ...
    aliases: ['dgiveaway', 'deletegiveaway'],
        ...
        if(message.guild.id === config["main_config"].yourserverid) {
            ...
            if(message.member.roles.cache.some(h=>per.includes(h.id))){
                ...
                if(config.giveaways_config.enabled) {
                    ...
                    if(!args[0]) return message.channel.send(`Error: Wrong Format, Try: \`${config.main_config.prefix}gdelete <giveawayMessageID>\``).then(msg => {
